...
const FROG_TONGUE = preload("res://Scenes/Enemies/Effects/Frog Tongue Vertical.tscn")
...
func walk():
	...
	velocity.y = velocity.lerp(Vector2(0, sign(direction.y) * _enemy_stats.speed), _enemy_stats.acceleration).y
	velocity.x = fall_direction.x * 1000
...
func spawn_tongue():
	...
	add_child(tongue)
	tongue.position.x = $Down.position.x * -fall_direction.x
	launch_delay.start()

func launch():
	var launch_speed = _enemy_stats.speed * 25
	velocity = velocity.lerp(Vector2(launch_speed * -fall_direction.x, 0), _enemy_stats.acceleration)
	...
	if collision:
		if tongue and fall_direction.is_equal_approx(collision.get_normal()):
			...
			for i in NUMBER_OF_BULLETS:
				...
				bullet.direction = Vector2.UP.rotated(angle) * sign(fall_direction)
				bullet.range = 300
				bullet.speed = 450
