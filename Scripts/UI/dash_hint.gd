...
func _physics_process(delta):
	...
	if InputMap.has_action("dash"):
		if not InputMap.action_get_events("dash").is_empty():
			var event = InputMap.action_get_events("dash")[0]
	...
	if InputMap.has_action("dash"):
		if InputMap.action_get_events("dash").size() > 1:
			var event = InputMap.action_get_events("dash")[1]
