...
func _physics_process(delta):
	...
	if InputMap.has_action("stat_sheet"):
		if not InputMap.action_get_events("stat_sheet").is_empty():
			var event = InputMap.action_get_events("stat_sheet")[0]
	...
	if InputMap.has_action("stat_sheet"):
		if InputMap.action_get_events("stat_sheet").size() > 1:
			var event = InputMap.action_get_events("stat_sheet")[1]
