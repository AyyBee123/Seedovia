...
func _physics_process(delta):
	...
	if InputMap.has_action("inventory"):
		if not InputMap.action_get_events("inventory").is_empty():
			var event = InputMap.action_get_events("inventory")[0]
	...
	if InputMap.has_action("inventory"):
		if InputMap.action_get_events("inventory").size() > 1:
			var event = InputMap.action_get_events("inventory")[1]
