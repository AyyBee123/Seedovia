...
func _physics_process(delta):
	...
	if InputMap.has_action("inventory_use"):
		if not InputMap.action_get_events("inventory_use").is_empty():
			var event = InputMap.action_get_events("inventory_use")[0]
	...
	if InputMap.has_action("inventory_use"):
		if not InputMap.action_get_events("inventory_use").is_empty():
			var event = InputMap.action_get_events("inventory_use")[1]
